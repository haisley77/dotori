<script setup>
  ...
  const joinRoom = () => {
    ...
    let canvasStream = canvas.captureStream(30); // 30은 FPS를 의미합니다.
    console.log("==================================================="+canvasStream.active)
    ...
    session.connect(token.value).then(() => {
      ...
      const publisher = OV.initPublisher(undefined, {insertMode: 'APPEND'});
      //내 영상을 내보낸다
      session.publish(myPublisher).then(() => {
      });
...
<template>
  ...
  <video ref="videoPlayer" id="videoPlayer" autoplay></video>
</template>
<style scoped>
  #videoPlayer {
    display: none;
